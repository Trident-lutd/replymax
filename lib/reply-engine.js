export function detectScenario(text = "") {
  ...
  const signals = {
    shortReply: /(\bk\b|\bok\b|haha|lol|nice|cool|maybe|we'?ll see|we will see)/.test(t),
    noReply: /(seen|left on read|ignored|no reply|didn'?t answer|didnt answer|ghosted)/.test(t),
...
export function fallbackReply(input = "", tone = "confident") {
  ...
  return library[scenario][tone] || library.general.confident;
}
